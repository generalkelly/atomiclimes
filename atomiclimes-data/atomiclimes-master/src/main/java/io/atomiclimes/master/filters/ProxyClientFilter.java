...
public class ProxyClientFilter implements AtomicLimesClientFilter {
	...
	private static final AtomicLimesLogger LOG = new AtomicLimesLogger(ProxyClientFilter.class);

	public ProxyClientFilter(AtomicLimesClientRegistry registry, AtomicLimesMasterProperties properties) {
	...
	public AtomicLimesRegistrationResponse filter(AtomicLimesClient client) {
		...
		registry.subscribe(ClientType.GUI, client);
